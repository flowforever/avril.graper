...
extend(Graper.prototype, {
    ...
    , parsePage: function (url, currentDone) {
        ...
        this.requestPage(url, function (err, res, pageBody) {
            currentDone();
                    ...
                    if (self.isListPage($)) {
                        ...
                        if (options.hasDetailOnList) {
                            ...
                            dataList.each(function () {
                                ...
                                //get detail pageData
                                detailUrl &&  self.parsePage( detailUrl );
                            });
                        }
                        ...
                        if (nextListLinks && nextListLinks.length) {
                            nextListLinks.each(function(){
                                ...
                                nextLink && self.parsePage(nextLink);
                            })
                        }else{
    ...
    , requestPage: function (url, callback) {
        ...
        request({
            ...
            clearTimeout( self.timeoutCache[url] );
            self.timeoutCache[url] = null;
            callback(err, res, pageBody);
        });

        this.timeoutCache[url] = setTimeout(function() {
            self.log('RETRY' , url);
            self.parseCache[url] = null;
            self.parsePage(url);
        }, this.options.retryTimeout + 10);

        return this;
    }
    , save: function (data) {
        Graper.db(this.options.dbType)(this, data);
    }
    , log: function () {
