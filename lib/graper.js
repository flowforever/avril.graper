...
extend(Graper.prototype, {
    ...
    , requestPage: function (url, callback) {
        if (this.options.engine === 'phantom') {
            return this.usePhantom(url, callback);
        } else {
            return this.useRequest(url, callback);
        }
    }
    ...
    , usePhantom: (function () {
        return function (url, callback) {
            var self = this;
            createPhantom(function (page) {
                page.open(url, function (status) {
                    if (status !== 'success') {
                        console.log('RETRY', url);
                        self.requestPage(url, callback);
                    } else {
    ...
    , useRequest: function (url, callback) {
